...
const USE_REMOTE_JUDGE = false; // Set to false since Piston API is discontinued. Using local compilers (javac, g++, gcc) instead.
...
`;
  ...
  try {
    ...
    return { success: true, output: JSON.parse(extractLastJson(stdout) || "null"), error: stderr };
  } catch (err) {
    return { success: false, error: err.stderr || err.message };
  }
}
