...
export async function func_982347(
  code,
  language,
  input = null,
  timeout = 5000,
...
async function runNode(code, input, timeout) {
  ...
  try {
    ...
    if (sIdx === -1) throw new Error("No JSON");
    return {
      success: true,
      output: JSON.parse(stdout.substring(sIdx, stdout.lastIndexOf("}") + 1))
        .result,
      error: stderr,
    };
  } catch (err) {
